#ifndef DETPART_ASTAR_DATA_H
#define DETPART_ASTAR_DATA_H
...
void undone_filter(galois::LargeArray<bool> &done, bool small_filter = false) {
    ...
    cout << "acc count in undone_filter: " << acc_count << " remaining " << numValidNets - acc_count << endl;
}


Bool newRipupCheck_atomic_data(TreeEdge* treeedge, int ripup_threshold, int netID,
                          int edgeID, std::atomic<short int>* h_edges_local_usage, std::atomic<short int>* v_edges_local_usage){ 
  short *gridsX, *gridsY;
    ...
    for (i = 0; i < treeedge->route.routelen; i++) {
        if (gridsX[i] == gridsX[i + 1]) // a vertical edge
        {
            ...
            if(old_usage + v_edges[grid].red >= vCapacity - ripup_threshold) {
                ...
                break_edge = i;
                needRipup  = TRUE;
                v_edges_local_usage[grid].fetch_sub((short unsigned)1, std::memory_order_relaxed);
      ...
      else if (gridsY[i] == gridsY[i + 1]) // a horizontal edge
      {
            ...
            if(old_usage + h_edges[grid].red >= hCapacity - ripup_threshold){
                break_edge = i;
                needRipup  = TRUE;
                h_edges_local_usage[grid].fetch_sub((short unsigned)1, std::memory_order_relaxed);
    ...
    if (needRipup) {
      for (i = 0; i < treeedge->route.routelen; i++) {
        ...
        if (gridsX[i] == gridsX[i + 1]) // a vertical edge
        {
			...
			v_edges_local_usage[ymin * xGrid + gridsX[i]].fetch_sub(
              (short unsigned)1, std::memory_order_relaxed);
        ...
        else /// if(gridsY[i]==gridsY[i+1])// a horizontal edge
        {
            ...
            //  (short unsigned)1, std::memory_order_relaxed);
			h_edges_local_usage[gridsY[i] * (xGrid - 1) + xmin].fetch_sub(
              (short unsigned)1, std::memory_order_relaxed);
        }
      }
      return (TRUE);
...
void mazeRouteMSMDDetPart_Astar_Data(int iter, int expand, float costHeight,
  ...
  std::cout << "inside mazeRouteMSMDDetPart_Astar_Data astar_weight: " << astar_weight << endl;
  ...
  std::atomic<short int>* h_edges_local_usage;
  std::atomic<short int>* v_edges_local_usage;

  // galois::StatTimer roundtimer1("round");
  // roundtimer1.start();
  h_edges_local_usage = (std::atomic<short int>*)calloc(((xGrid - 1) * yGrid), sizeof(std::atomic<short int>));
  v_edges_local_usage = (std::atomic<short int>*)calloc((xGrid * (yGrid - 1)), sizeof(std::atomic<short int>)); //calloc is to init with 0s


  /*galois::do_all(galois::iterate((int)0, (xGrid - 1) * yGrid), [&](int c) {
    h_edges_local_usage[c] = 0;
  });
  ...
  for (int idx = 0; idx < parts; idx++) {
        ...
        [&](const auto nidRPC) {
          ...
          if (done[netID]) {
                ...
                mazeedge_Threshold) // only route the non-degraded edges (len>0)
            {
              ...
              enter = newRipupCheck_atomic_data(
                  treeedge, ripup_threshold, netID, edgeID, h_edges_local_usage,
                  v_edges_local_usage);
              ...
              if (enter) {
                ...
                for (i = 0; i < cnt_n1n2 - 1; i++) {
                  if (gridsX[i] == gridsX[i + 1]) // a vertical edge
                  {
                    ...
                    v_edges_local_usage[min_y * xGrid + gridsX[i]].fetch_add((short int)1);
                  } else /// if(gridsY[i]==gridsY[i+1])// a horizontal edge
                  {
                    min_x = min(gridsX[i], gridsX[i + 1]);
 
                    h_edges_local_usage[gridsY[i] * (xGrid - 1) + min_x].fetch_add((short int)1);
    ...
    galois::do_all(galois::iterate((int)0, (xGrid - 1) * yGrid), [&](int c) {
      if(h_edges_local_usage[c] != 0) {
        h_edges[c].usage += h_edges_local_usage[c];
        h_edges_local_usage[c] = 0;
      }
    });
    ...
    // update v_edges
    galois::do_all(galois::iterate((int)0, xGrid * (yGrid - 1)), [&](int c) {
      if(v_edges_local_usage[c] != 0) {
        v_edges[c].usage += v_edges_local_usage[c];
        v_edges_local_usage[c] = 0;
      }
    });
  ...
  free(h_edges_local_usage);
  free(v_edges_local_usage);
