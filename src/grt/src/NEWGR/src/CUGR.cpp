...
void CUGR::patternRoute(std::vector<int>& netIndices)
{
  ...
  sortNetIndices(netIndices, /*overflow_priority*/ false);
...
void CUGR::patternRouteWithDetours(std::vector<int>& netIndices)
{
  ...
  sortNetIndices(netIndices, /*overflow_priority*/ true);
  for (const int netIndex : netIndices) {
...
void CUGR::mazeRoute(std::vector<int>& netIndices)
{
  ...
  logger_->report(
      "stage 3 threshold filter: {} selected, {} skipped (threshold: {})",
      mazeNetIndices.size(),
      skippedByThreshold,
      mazeThreshold);
...
void CUGR::sortNetIndices(std::vector<int>& netIndices,
                          const bool overflow_priority) const
{
