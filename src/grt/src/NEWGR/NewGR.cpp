    ...
    MosaicHybridStats& stats)
{
  ...
  const int64_t total_via_increase_budget
      = std::max<int64_t>(6400, static_cast<int64_t>(seed_total_vias / 18));
  const int64_t total_low_layer_growth_budget
      = std::max<int64_t>(9200000, static_cast<int64_t>(seed_low_layer_wl / 26));
  const uint64_t wl_gain_target
      = std::max<uint64_t>(19000000, seed_wirelength / 24);
  const size_t swap_limit = std::min<size_t>(
      static_cast<size_t>(28000),
      std::max<size_t>(static_cast<size_t>(4500),
                       (seed_routes.size() * static_cast<size_t>(4)) / 5));
  const size_t min_swaps_before_stop
      = std::max<size_t>(180, db_net_map.size() / 70);
  ...
  for (auto& [db_net, base_route] : hybrid_routes) {
    ...
    const uint64_t min_gain = std::max<uint64_t>(8, minMosaicWirelengthGain(pin_count) / 3);
    const int64_t via_limit
        = maxMosaicViaIncrease(pin_count)
          + ((pin_count <= 12) ? 2 : ((pin_count <= 24) ? 4 : 6));
    ...
    for (const NetRouteMap* donor_routes : donor_route_sets) {
      ...
      if (via_increase > via_limit
          && wl_gain
                 < via_increase * 48 + low_layer_growth / 18
                       + static_cast<int64_t>(min_gain / 2 + 8)) {
        ++stats.skipped_by_via_guard;
        continue;
      }
      ...
      if (low_layer_growth > std::max<int64_t>(
                                 160,
                                 wl_gain * 3 / 2
                                     + static_cast<int64_t>(pin_count) * 32)
          && wl_gain
                 < low_layer_growth / 5 + via_increase * 40
                       + static_cast<int64_t>(min_gain)) {
        ++stats.skipped_by_layer_guard;
        continue;
      }

      const int64_t effective_gain = wl_gain * 14 + via_drop * 22
                                     + low_layer_release / 2 - via_increase * 20
                                     - low_layer_growth / 8;
  ...
  // Radical closure pass:
  // after the first mosaic sweep, revisit nets and pick shortest donors from
  // all route banks under relaxed (but still bounded) via/low-layer budgets.
  // This intentionally prioritizes total wirelength reduction.
  struct ClosureCandidate
  {
    ...
    int64_t via_increase{0};
    int64_t via_drop{0};
    int64_t low_layer_growth{0};
    int64_t priority{0};
  };

  std::vector<ClosureCandidate> closure_candidates;
  closure_candidates.reserve(db_net_map.size());
  ...
  for (const auto& [db_net, net] : db_net_map) {
    ...
    const GRoute* best_donor_route = nullptr;
    RouteScore best_donor_score;
    RouteLayerUsage best_donor_usage;
    ...
    for (const NetRouteMap* donor_routes : donor_route_sets) {
      ...
      if (best_donor_route == nullptr
          || std::make_tuple(donor_score.wirelength,
                 ...
                 < std::make_tuple(best_donor_score.wirelength,
                                   best_donor_score.vias,
                                   best_donor_usage.low_layer_wl)) {
        best_donor_route = &donor_it->second;
        best_donor_score = donor_score;
        best_donor_usage = donor_usage;
      }
    }
    ...
    const int64_t wl_gain = static_cast<int64_t>(base_score.wirelength)
                            - static_cast<int64_t>(best_donor_score.wirelength);
    const int64_t via_increase = std::max<int64_t>(
        0,
        static_cast<int64_t>(best_donor_score.vias)
            - static_cast<int64_t>(base_score.vias));
    const int64_t via_drop = std::max<int64_t>(
        0,
        static_cast<int64_t>(base_score.vias)
    ...
    const int64_t low_layer_growth = std::max<int64_t>(
        0,
        static_cast<int64_t>(best_donor_usage.low_layer_wl)
            - static_cast<int64_t>(base_usage.low_layer_wl));

    const int64_t via_limit
        = maxMosaicViaIncrease(pin_count)
          + ((pin_count <= 12) ? 4 : ((pin_count <= 24) ? 6 : 8));
    const int64_t min_closure_gain = std::max<int64_t>(
        4, static_cast<int64_t>(minClosureWirelengthGain(pin_count) / 2));
    if (wl_gain < min_closure_gain) {
      continue;
    }
    if (via_increase > via_limit
        && wl_gain
               < via_increase * 36 + low_layer_growth / 24
    ...
    if (low_layer_growth > std::max<int64_t>(
                               72,
                               wl_gain * 2 + static_cast<int64_t>(pin_count) * 20)
        && wl_gain
               < low_layer_growth / 6 + via_increase * 32
                     + static_cast<int64_t>(20)) {
      ++stats.skipped_by_layer_guard;
      continue;
    }
    ...
    const int64_t priority
        = wl_gain * 16 + via_drop * 12 - via_increase * 14
          - low_layer_growth / 10;
    closure_candidates.push_back({db_net,
                                  best_donor_route,
                                  pin_count,
                                  wl_gain,
                                  via_increase,
  ...
  std::sort(closure_candidates.begin(),
            closure_candidates.end(),
            [](const ClosureCandidate& lhs, const ClosureCandidate& rhs) {
              return std::make_tuple(lhs.priority,
                                     lhs.wl_gain,
                                     std::max<int64_t>(0, lhs.via_drop),
                                     -std::max<int64_t>(0, lhs.via_increase),
                                     -std::max<int64_t>(0, lhs.low_layer_growth))
                     > std::make_tuple(rhs.priority,
                                       rhs.wl_gain,
                                       std::max<int64_t>(0, rhs.via_drop),
                                       -std::max<int64_t>(0, rhs.via_increase),
                                       -std::max<int64_t>(0, rhs.low_layer_growth));
            });

  const size_t closure_swap_limit = std::min<size_t>(
      static_cast<size_t>(12000),
      std::max<size_t>(static_cast<size_t>(1200), db_net_map.size() / 2));
  const int64_t closure_via_budget
      = std::max<int64_t>(2400, static_cast<int64_t>(seed_total_vias / 46));
  const int64_t closure_low_layer_budget
      = std::max<int64_t>(1800000, static_cast<int64_t>(seed_low_layer_wl / 120));
  const int64_t closure_total_via_budget
      = total_via_increase_budget + closure_via_budget;
  const int64_t closure_total_low_layer_budget
      = total_low_layer_growth_budget + closure_low_layer_budget;
  const uint64_t closure_wl_target
      = std::max<uint64_t>(3000000, seed_wirelength / 90);
  const size_t closure_min_swaps
      = std::max<size_t>(120, closure_swap_limit / 10);

  uint64_t closure_wl_gain = 0;
  size_t closure_swaps = 0;
  for (const ClosureCandidate& candidate : closure_candidates) {
    if (closure_swaps >= closure_swap_limit) {
      break;
    }
    ...
    const int64_t via_limit
        = maxMosaicViaIncrease(candidate.pin_count)
          + ((candidate.pin_count <= 12) ? 4 : ((candidate.pin_count <= 24) ? 6 : 8));
    if (via_increase > via_limit
        && wl_gain
    ...
    if (low_layer_growth > std::max<int64_t>(
                               96,
                               wl_gain * 2 + static_cast<int64_t>(candidate.pin_count) * 24)
        && wl_gain
               < low_layer_growth / 5 + via_increase * 30
                     + static_cast<int64_t>(16)) {
      ++stats.skipped_by_layer_guard;
      continue;
    }
    if (consumed_via_increase + via_increase > closure_total_via_budget
        || consumed_low_layer_growth + low_layer_growth
               > closure_total_low_layer_budget) {
      ++stats.skipped_by_budget_guard;
      continue;
    }
    ...
    closure_wl_gain += net_wl_gain;
    ++stats.replaced_with_donor;
    ++closure_swaps;
    if (closure_wl_gain >= closure_wl_target && closure_swaps >= closure_min_swaps) {
      break;
    }
  }
    ...
    WirelengthFusionStats& stats)
{
  NetRouteMap hybrid_routes = seed_routes;
  const uint64_t seed_total_wirelength = computeRouteScore(seed_routes).wirelength;
  // Wirelength-majority fusion:
  // mix all donor families with soft-cap-like guards, then run a second
  // localized closure pass to capture residual per-net WL wins.
  const int64_t via_increase_budget = std::max<int64_t>(
  ...
  // Second pass: per-net micro-fusion closure.
  // This borrows SPRoute-style staged refinement while keeping CUGR-like
  // low-layer/via soft budgets. We only accept strict per-net WL wins.
  ...
  std::vector<MicroCandidate> micro_candidates;
  micro_candidates.reserve(db_net_map.size());
  ...
  for (const auto& [db_net, net] : db_net_map) {
    ...
    for (const NetRouteMap* donor_set : donor_route_sets) {
      ...
      const int64_t via_soft_limit
          = maxInterleavedViaIncrease(pin_count)
            + ((pin_count <= 12) ? 8 : ((pin_count <= 32) ? 12 : 16));
      if (via_increase > via_soft_limit
          && wl_gain
                 < via_increase * 44 + low_layer_growth / 14
                       + static_cast<int64_t>(20)) {
        continue;
      }
      if (low_layer_growth > 0
          && wl_gain
                 < low_layer_growth * 2
                       + std::max<int64_t>(0, via_increase) * 24
                       + static_cast<int64_t>(80)) {
        continue;
      }
    ...
    const int64_t low_layer_growth = std::max<int64_t>(0, low_layer_delta);
    const int64_t priority
        = wl_gain * 6 + std::max<int64_t>(0, via_drop) * 16
          + low_layer_release / 4 - std::max<int64_t>(0, via_increase) * 24
          - low_layer_growth / 2;
    micro_candidates.push_back({db_net,
                                best_donor,
                                pin_count,
                                wl_gain,
                                via_increase,
  ...
  std::sort(micro_candidates.begin(),
            micro_candidates.end(),
            [](const MicroCandidate& lhs, const MicroCandidate& rhs) {
              return std::make_tuple(lhs.priority,
                                     lhs.wl_gain,
                                     std::max<int64_t>(0, lhs.via_drop),
                                     -std::max<int64_t>(0, lhs.via_increase),
  ...
  const size_t micro_swap_limit = std::min<size_t>(
      2400, std::max<size_t>(300, db_net_map.size() / 7));
  const int64_t micro_via_budget = std::max<int64_t>(
      1600, static_cast<int64_t>(seed_total_vias / 58));
  const int64_t micro_low_layer_budget = std::max<int64_t>(
      800000, static_cast<int64_t>(seed_low_layer_wl / 140));
  const int64_t total_via_budget = via_increase_budget + micro_via_budget;
  const int64_t total_low_layer_budget
      = low_layer_growth_budget + micro_low_layer_budget;
  const uint64_t micro_wl_target = std::max<uint64_t>(150000, seed_total_wirelength / 4000);
  const size_t micro_min_swaps = std::max<size_t>(60, micro_swap_limit / 8);

  uint64_t micro_wl_gain = 0;
  size_t micro_swaps = 0;
  for (const MicroCandidate& candidate : micro_candidates) {
    if (micro_swaps >= micro_swap_limit) {
      break;
    }
    auto base_it = hybrid_routes.find(candidate.db_net);
    if (base_it == hybrid_routes.end() || base_it->second.empty()) {
    ...
    const int64_t low_layer_release = std::max<int64_t>(
        ...
        static_cast<int64_t>(live_base_usage.low_layer_wl)
            - static_cast<int64_t>(donor_usage.low_layer_wl));
    const int64_t effective_gain
        = wl_gain * 3 + low_layer_release / 2 - via_increase * 28
          - low_layer_growth / 4;

    if (via_increase > 0
        && effective_gain
               < via_increase * 44 + low_layer_growth / 16
                     + static_cast<int64_t>(12)) {
      ++stats.skipped_by_via_guard;
      continue;
    }
    if (low_layer_growth > 0
        && effective_gain
               < low_layer_growth * 2 + via_increase * 20
                     + static_cast<int64_t>(20)) {
      ++stats.skipped_by_layer_guard;
      continue;
    }
    ...
    if (consumed_via_increase + via_increase > total_via_budget
        || consumed_low_layer_growth + low_layer_growth > total_low_layer_budget) {
      ++stats.skipped_by_budget_guard;
      continue;
    }
    ...
    const uint64_t net_wl_gain = static_cast<uint64_t>(std::max<int64_t>(0, wl_gain));
    consumed_wl_gain += net_wl_gain;
    micro_wl_gain += net_wl_gain;
    ++stats.replaced_with_donor;
    ++micro_swaps;
    if (micro_wl_gain >= micro_wl_target && micro_swaps >= micro_min_swaps) {
      break;
    }
  }
  ...
  // constraining via/low-layer growth.
  ...
  for (const auto& [db_net, net] : db_net_map) {
    ...
    for (const NetRouteMap* donor_set : donor_route_sets) {
      ...
      const int64_t via_soft_limit
          = maxInterleavedViaIncrease(pin_count)
            + ((pin_count <= 12) ? 10 : ((pin_count <= 32) ? 14 : 18));
      if (via_increase > via_soft_limit
          && wl_gain
                 < via_increase * 42 + low_layer_growth * 2
                       + static_cast<int64_t>(40)) {
        continue;
      }
      if (low_layer_growth > 0
          && (low_layer_growth
                  > std::max<int64_t>(
                        96,
                        wl_gain / 3 + static_cast<int64_t>(pin_count) * 6)
              || wl_gain
                     < low_layer_growth * 4
                           + std::max<int64_t>(0, via_increase) * 30
                           + static_cast<int64_t>(90))) {
        continue;
      }
    ...
    const int64_t low_layer_growth = std::max<int64_t>(0, low_layer_delta);
    const int64_t priority
        = wl_gain * 8 + std::max<int64_t>(0, via_drop) * 10 + low_layer_release
          - std::max<int64_t>(0, via_increase) * 26 - low_layer_growth * 3;
  ...
  const size_t final_swap_limit = std::min<size_t>(
      2600, std::max<size_t>(260, db_net_map.size() / 8));
  const int64_t final_via_budget = std::max<int64_t>(
      900, static_cast<int64_t>(seed_total_vias / 95));
  const int64_t final_low_layer_budget = std::max<int64_t>(
      90000, static_cast<int64_t>(seed_low_layer_wl / 1200));
  ...
  const uint64_t final_wl_target = std::max<uint64_t>(120000, seed_total_wirelength / 5200);
  const size_t final_min_swaps = std::max<size_t>(40, final_swap_limit / 12);
  ...
  for (const FinalClosureCandidate& candidate : final_candidates) {
    ...
    const int64_t low_layer_release = std::max<int64_t>(
        ...
        static_cast<int64_t>(live_base_usage.low_layer_wl)
            - static_cast<int64_t>(donor_usage.low_layer_wl));
    const int64_t effective_gain
        = wl_gain * 5 + low_layer_release - via_increase * 26
          - low_layer_growth * 3;

    if (via_increase > 0
        && effective_gain
               < via_increase * 62 + low_layer_growth * 2
    ...
    if (low_layer_growth > 0
        && (low_layer_growth
                > std::max<int64_t>(
                      72,
                      wl_gain / 4 + static_cast<int64_t>(candidate.pin_count) * 6)
            || effective_gain
                   < low_layer_growth * 4 + via_increase * 28
                         + static_cast<int64_t>(90))) {
      ++stats.skipped_by_layer_guard;
      continue;
    }
  ...
  // Fourth pass: radical shortest-net consensus closure.
  // Re-run a wirelength-first sweep using the best donor per net across all
  // banks with relaxed (but still bounded) via/layer guards.
  struct RadicalConsensusCandidate
  {
    odb::dbNet* db_net{nullptr};
    const GRoute* donor_route{nullptr};
    int pin_count{0};
  ...
  std::vector<RadicalConsensusCandidate> radical_candidates;
  radical_candidates.reserve(db_net_map.size());
  ...
  for (const auto& [db_net, net] : db_net_map) {
    ...
    const int64_t low_layer_growth = std::max<int64_t>(0, low_layer_delta);
    const int64_t priority
        = wl_gain * 12 + std::max<int64_t>(0, via_drop) * 8
          + low_layer_release / 2 - std::max<int64_t>(0, via_increase) * 14
          - low_layer_growth / 12;

    radical_candidates.push_back({db_net,
                                  best_donor,
                                  pin_count,
                                  wl_gain,
                                  via_increase,
  ...
  std::sort(radical_candidates.begin(),
            radical_candidates.end(),
            [](const RadicalConsensusCandidate& lhs,
               const RadicalConsensusCandidate& rhs) {
              return std::make_tuple(lhs.priority,
                                     lhs.wl_gain,
                                     std::max<int64_t>(0, lhs.via_drop),
                                     -std::max<int64_t>(0, lhs.via_increase),
  ...
  const size_t radical_swap_limit = std::min<size_t>(
      5200, std::max<size_t>(520, db_net_map.size() / 5));
  const int64_t radical_via_budget = std::max<int64_t>(
      2600, static_cast<int64_t>(seed_total_vias / 40));
  const int64_t radical_low_layer_budget = std::max<int64_t>(
      1200000, static_cast<int64_t>(seed_low_layer_wl / 320));
  const int64_t radical_total_via_budget = final_total_via_budget + radical_via_budget;
  const int64_t radical_total_low_layer_budget
      = final_total_low_layer_budget + radical_low_layer_budget;
  const uint64_t radical_wl_target = std::max<uint64_t>(
      600000, seed_total_wirelength / 1800);
  const size_t radical_min_swaps
      = std::max<size_t>(90, radical_swap_limit / 12);

  uint64_t radical_wl_gain = 0;
  size_t radical_swaps = 0;
  for (const RadicalConsensusCandidate& candidate : radical_candidates) {
    if (radical_swaps >= radical_swap_limit) {
      break;
    }
    auto base_it = hybrid_routes.find(candidate.db_net);
    if (base_it == hybrid_routes.end() || base_it->second.empty()) {
    ...
    const int64_t low_layer_growth = std::max<int64_t>(
        ...
        static_cast<int64_t>(donor_usage.low_layer_wl)
            - static_cast<int64_t>(live_base_usage.low_layer_wl));
    const int64_t via_soft_limit
        = maxInterleavedViaIncrease(candidate.pin_count)
          + ((candidate.pin_count <= 12) ? 12 : ((candidate.pin_count <= 32) ? 18 : 24));

    if (via_increase > via_soft_limit
        && wl_gain
               < via_increase * 34 + low_layer_growth / 10
    ...
    if (low_layer_growth > 0
        && wl_gain
               < low_layer_growth / 8 + via_increase * 20
                     + static_cast<int64_t>(16)) {
      ++stats.skipped_by_layer_guard;
      continue;
    }
    if (via_increase > 0
        && low_layer_growth
               > wl_gain * 3 + static_cast<int64_t>(candidate.pin_count) * 40) {
      ++stats.skipped_by_layer_guard;
      continue;
    }
    if (consumed_via_increase + via_increase > radical_total_via_budget
        || consumed_low_layer_growth + low_layer_growth
               > radical_total_low_layer_budget) {
      ++stats.skipped_by_budget_guard;
      continue;
    }
    ...
    const uint64_t net_wl_gain = static_cast<uint64_t>(std::max<int64_t>(0, wl_gain));
    consumed_wl_gain += net_wl_gain;
    radical_wl_gain += net_wl_gain;
    ++stats.replaced_with_donor;
    ++radical_swaps;
    if (radical_wl_gain >= radical_wl_target && radical_swaps >= radical_min_swaps) {
      break;
    }
  }
                                    ...
                                    uint64_t candidate_low_layer_wl)
{
  ...
  const uint64_t min_wl_gain
      = std::max<uint64_t>(160, incumbent_score.wirelength / 1400000);
  if (wl_gain < min_wl_gain) {
    return false;
  }
  const int64_t via_increase_budget
      = std::max<int64_t>(4800, static_cast<int64_t>(incumbent_score.vias / 14));
  if (via_increase > via_increase_budget
      && wl_gain
             < static_cast<uint64_t>(via_increase * 34 + static_cast<int64_t>(600))) {
    return false;
  }
  const int64_t low_layer_budget = std::max<int64_t>(
      4800000, static_cast<int64_t>(incumbent_low_layer_wl / 34));
  if (low_layer_delta > low_layer_budget
      && wl_gain
             < static_cast<uint64_t>(low_layer_delta / 24 + static_cast<int64_t>(1200))) {
    return false;
  }
  if (low_layer_delta > 0 && via_increase > 0
      && wl_gain
             < static_cast<uint64_t>(low_layer_delta / 36
                                     + via_increase * 40
                                     + static_cast<int64_t>(1600))) {
    return false;
  }
  return true;
}
                                        ...
                                        uint64_t candidate_low_layer_wl)
{
  ...
  const bool strong_risk_drop
      = low_layer_delta <= -40000
        && (via_increase <= 0 || via_increase <= 320);
  const uint64_t min_wl_gain
      = std::max<uint64_t>(6000, incumbent_score.wirelength / 120000);
  if (effective_wl_gain < min_wl_gain
      && !(strong_risk_drop && wl_gain >= min_wl_gain / 2)) {
    return false;
  }
  const int64_t via_increase_budget
      = std::max<int64_t>(1800, static_cast<int64_t>(incumbent_score.vias / 80));
  if (via_increase > via_increase_budget
      && effective_wl_gain
             < static_cast<uint64_t>(via_increase * 190 + static_cast<int64_t>(24000))) {
    return false;
  }
  if (via_increase > 0
      && effective_wl_gain
             < std::max<uint64_t>(
                 static_cast<uint64_t>(via_increase * 160
                                       + static_cast<int64_t>(24000)),
                 static_cast<uint64_t>(26000))) {
    return false;
  }
  const int64_t low_layer_budget = std::max<int64_t>(
      3200000, static_cast<int64_t>(incumbent_low_layer_wl / 70));
  if (low_layer_delta > low_layer_budget
      || (low_layer_delta > 0
          && effective_wl_gain
                 < static_cast<uint64_t>(low_layer_delta / 2
                                         + static_cast<int64_t>(35000)))) {
    return false;
  }
  return true;
}
                                         ...
                                         uint64_t candidate_low_layer_wl)
{
  ...
  const uint64_t effective_wl_gain = wl_gain + low_layer_release_credit;
  const bool strong_risk_drop
      = low_layer_delta <= -50000
        && (via_increase <= 0 || via_increase <= 300);
  const uint64_t min_wl_gain
      = std::max<uint64_t>(5000, incumbent_score.wirelength / 130000);
  if (effective_wl_gain < min_wl_gain
      && !(strong_risk_drop && wl_gain >= min_wl_gain / 2)) {
    return false;
  }
  const int64_t via_increase_budget
      = std::max<int64_t>(1400, static_cast<int64_t>(incumbent_score.vias / 100));
  if (via_increase > via_increase_budget
      && effective_wl_gain
             < static_cast<uint64_t>(via_increase * 220 + static_cast<int64_t>(26000))) {
    return false;
  }
  if (via_increase > 0
      && effective_wl_gain
             < std::max<uint64_t>(
                 static_cast<uint64_t>(via_increase * 180
                                       + static_cast<int64_t>(26000)),
                 static_cast<uint64_t>(30000))) {
    return false;
  }
  const int64_t low_layer_budget = std::max<int64_t>(
      2600000, static_cast<int64_t>(incumbent_low_layer_wl / 75));
  if (low_layer_delta > low_layer_budget
      && effective_wl_gain
             < static_cast<uint64_t>(low_layer_delta / 2
                                        ...
                                        uint64_t candidate_low_layer_wl)
{
  ...
  const uint64_t low_layer_release_credit = static_cast<uint64_t>(
      std::max<int64_t>(0, -low_layer_delta));
  const int64_t effective_gain_i64
      = static_cast<int64_t>(wl_gain * 2 + low_layer_release_credit)
        - low_layer_growth / 2;
  const uint64_t effective_wl_gain
      = static_cast<uint64_t>(std::max<int64_t>(0, effective_gain_i64));
  const uint64_t min_wl_gain
      = std::max<uint64_t>(240, incumbent_score.wirelength / 1200000);
  if (effective_wl_gain < min_wl_gain) {
    return false;
  }

  const int64_t via_increase_budget
      = std::max<int64_t>(6000, static_cast<int64_t>(incumbent_score.vias / 18));
  if (via_growth > via_increase_budget
      && effective_wl_gain
             < static_cast<uint64_t>(via_growth * 54 + static_cast<int64_t>(6000))) {
    return false;
  }
  if (via_growth > 0
      && effective_wl_gain
             < std::max<uint64_t>(
                 ...
                 static_cast<uint64_t>(2400))) {
    return false;
  }

  const int64_t low_layer_budget = std::max<int64_t>(
      4800000, static_cast<int64_t>(incumbent_low_layer_wl / 36));
  if (low_layer_delta > low_layer_budget
      && effective_wl_gain
             < static_cast<uint64_t>(low_layer_delta / 4
                                     + via_growth * 80
                                     + static_cast<int64_t>(24000))) {
    return false;
  }
                       ...
                       int max_routing_layer)
{
  ...
  if (!disable_fastroute_graft && grouter_->fastroute() != nullptr) {
    ...
    const uint64_t mosaic_added
        = tempered_added + mosaic_stats.added_missing_nets;
    ...
    if (shouldPreferWirelengthOracleHybrid(best_overflow,
                                           ...
                                           wirelength_oracle_score,
                                           wirelength_oracle_low_layer_wl)
        && best_score.wirelength > wirelength_oracle_score.wirelength
        && best_score.wirelength - wirelength_oracle_score.wirelength
               >= oracle_min_gain) {
    ...
    if (shouldPreferWirelengthClosureHybrid(best_overflow,
                                            best_score,
                                            best_low_layer_wl,
                                            last_total_overflow_,
                                            wirelength_closure_score,
                                            wirelength_closure_low_layer_wl)
        && best_score.wirelength > wirelength_closure_score.wirelength
        && best_score.wirelength - wirelength_closure_score.wirelength
               >= closure_min_gain) {
    ...
    if (shouldPreferWirelengthFusionHybrid(best_overflow,
                                           ...
                                           wirelength_fusion_score,
                                           wirelength_fusion_low_layer_wl)
        && best_score.wirelength > wirelength_fusion_score.wirelength
        && best_score.wirelength - wirelength_fusion_score.wirelength
               >= fusion_min_gain) {
    ...
    if (shouldPreferWirelengthFusionHybrid(best_overflow,
        ...
        && best_score.wirelength - tempered_fusion_score.wirelength
               >= tempered_min_gain) {
      routes = std::move(tempered_fusion_hybrid);
      best_score = tempered_fusion_score;
      best_overflow = last_total_overflow_;
      best_low_layer_wl = tempered_fusion_low_layer_wl;
      selected_label = "FastRoute+NEWGR tempered-fusion";
      selected_hybrid = true;
      selected_swapped_nets = tempered_swapped;
      selected_added_nets = tempered_added;
      used_fastroute_last_run_ = false;
    }
    ...
    if (shouldPreferWirelengthChampion(best_overflow,
      ...
      best_overflow = last_total_overflow_;
      best_low_layer_wl = tempered_fusion_low_layer_wl;
      selected_label = "FastRoute+NEWGR wl-champion-tempered";
      selected_hybrid = true;
      selected_swapped_nets = tempered_swapped;
      selected_added_nets = tempered_added;
      used_fastroute_last_run_ = false;
