...
DTYPE flute_wl(int d, DTYPE x[], DTYPE y[], int acc) {
  ...
  int s[MAXD];
  int i, j, k, minidx;
  struct point_flute pt[MAXD], *ptp[MAXD], *tmpp;
...
Tree flute(int d, DTYPE x[], DTYPE y[], int acc) {
  ...
  int* s;
  int i, j, k, minidx;
  struct point_flute *pt, **ptp, *tmpp;
  Tree t;
  ...
  if (d == 2) {
    ...
    pt  = (struct point_flute*)malloc(sizeof(struct point_flute) * (d + 1));
    ptp = (struct point_flute**)malloc(sizeof(struct point_flute*) * (d + 1));
    ...
    if (d < 200) {
      ...
      qsort(ptp, d, sizeof(struct point_flute*), orderx);
    }
    ...
    if (d < 200) {
      ...
      qsort(ptp, d, sizeof(struct point_flute*), ordery);
      for (i = 0; i < d; i++) {
        ys[i] = ptp[i]->y;
        s[i]  = ptp[i]->o;
      }
...
Tree flutes_MD(int d, DTYPE xs[], DTYPE ys[], int s[], int acc) {
  ...
  if (s[0] < s[d - 1]) {
    ...
    for (i = 2; i <= ms; i++)
      ms = max(ms, s[i]);
    if (ms <= d - 3 && d < 15) { //Michael: for d > 1000
      for (i = 0; i <= ms; i++) {
        x1[i] = xs[i];
        y1[i] = ys[i];
        s1[i] = s[i];
