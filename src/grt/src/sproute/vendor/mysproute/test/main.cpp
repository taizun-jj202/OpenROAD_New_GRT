...
void GcellInsertOBS(int x, int y, int z, Rect2D<int>& rect, int OBSIdx)
{
    ...
    /*if(localEnd >= gcell.numTracks || localStart < 0) {
        cout << OBSTrackRange.start << " " << OBSTrackRange.end << endl;
        cout << gcellTrackStart << endl;
        cout << gcellTrackEnd << endl;
        cout << " end exceed track used size: " << localEnd << " " <<  gcell.numTracks << " " << localStart << endl;
        exit(1);
    }*/
...
void preprocessDesignRuleOBS() //TODO: this function has a bug on ispd18_test10
{
    ...
    for(int i = 0; i <  lefDB.layers.size(); i++)
    {
        ...
        for(int OBSIdx = 0; OBSIdx < defDB.origOBS.at(i).size(); OBSIdx++)
        {
            ...
            if(width != lefDB.layers.at(i).width * dbuPerMicro) { //not a standard-width wire
                ...
                expand = std::max(expand, COR_expand);// + lefDB.layers.at(i).width * dbuPerMicro;
                ...
                /*if(layerName == "Metal5" && y_diff > lefDB.layers.at(i).pitchx * dbuPerMicro * 2000) {
                    cout << " wtf? " << lefDB.layers.at(i).pitchx * dbuPerMicro * 1000 << " " << y_diff << endl;
                    cout << "original: " << endl;
                    rect.print();
                    cout << " expanded: " << endl;
                    expandRect.print();
                    cout << "PRL: " << col << " " << row << " " << PRL_expand << endl;
                    exit(1);
...
{
    string defFileName, lefFileName, outFileName, algo_str, mode, grFileName;
    numThreads = 1;
    ...
    if(mode == "lefdef") {
        ...
        if(grFileName.size() == 0) {
            cout << "usage: ./sproute -mode gr -gr [grFileName]" << endl;
            exit(1);
        }
    }
    ...
    galois::StatTimer breaktimer("break");
    ...
    if(mode == "lefdef") {
        ...
        breaktimer.stop();
        cout << "===reading time: " << breaktimer.get() << endl;
        breaktimer.start();
        ...
        breaktimer.stop();
        cout << "===preprocess time: " << breaktimer.get() << endl;
        breaktimer.start();

        adjustGcellCap(graphTemplates, capReductions);

        grGen = grGenerator(lefDB, defDB, capReductions);
        //grGen.write("temp.gr");

         congestionMap = CongestionMap(grGen.grid.z, grGen.grid.x, grGen.grid.y);

        breaktimer.stop();
        cout << "===grgen time: " << breaktimer.get() << endl;
        breaktimer.start();
    }
    else if(mode == "gr") {
    ...
    runFastRoute(grGen, grFileName, outFileName, congestionMap, breaktimer, 30, algo);
