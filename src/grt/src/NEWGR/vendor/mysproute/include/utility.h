...
void assignEdge(int netID, int edgeID, Bool processDIR) 
{
	...
	// Escalate layer-assignment via penalties only at very high global via
	// cost so wirelength-driven paths stay available in moderate congestion.
	const int la_via_start = (viacost >= 5) ? 3 : 2;
	const int la_via_mid = (viacost >= 5) ? 4 : 3;
	const int la_via_end = (viacost >= 5) ? 2 : 1;
	...
	if (processDIR) {
		...
		for (l = 0 ; l < numLayers; l ++) {
			for (i = 0 ; i < numLayers; i++) {
				if (l != i) {
					if (gridD[i][k] > gridD[l][k] + ADIFF(i,l) * la_via_end) {
						gridD[i][k] = gridD[l][k] + ADIFF(i,l) * la_via_end;
						viaLink[i][k] = l;
					}
				}
			}
...
static int compareLarge (const void *a, const void *b)
{
	if (((OrderTree*)a)->xmin > ((OrderTree*)b)->xmin) return 1;
    else if (((OrderTree*)a)->xmin == ((OrderTree*)b)->xmin) {
		if(a > b)	
...
int undoneNetOrderSize(galois::LargeArray<bool> &done)
{
	short *gridsX, *gridsY;
	int i, d, n1, n2, x1, y1, x2, y2, ind, l, grid, min_x, min_y;
	TreeEdge *treeedges, *treeedge;
		...
		for(ind=0; ind<2*d-3; ind++)
		{
            ...
            bool edge_done = checkIfDone(treeedge);
		  	...
		  	if(!edge_done) {
				gridsX = treeedge->route.gridsX;
				gridsY = treeedge->route.gridsY;
				treeOrderCong[undone_cnt].xmin = gridsX[i] + gridsX[treeedge->route.routelen];
				undone_cnt++;
				break;
			}
		}
...
void writeRoutedGrid(FILE* fp, std::set<parser::Point3D<int>>& routedGrid)
{
...
void writeRoutedGrid(FILE* fp, std::set<parser::Routed3DPoint>& routedGrid)
{
...
void writeRoute3D(char routingfile3D[], parser::grGenerator grGen)
{
	short *gridsX, *gridsY, *gridsL;
	int netID, d, i,k, edgeID,nodeID,deg, lastX, lastY,lastL, xreal, yreal,l, routeLen;
	TreeEdge *treeedges, *treeedge;
	...
	TreeNode *nodes;
	TreeEdge edge;
	using Point3D = parser::Point3D<int>;
	...
	for(netID=0;netID<numValidNets;netID++)
	{
		...
		std::set<parser::Routed3DPoint> routedGrid;
		...
		for(auto p : grNet.pinRegion)
		{
			...
			for (int layer = botL; layer <= topL; layer++) //this is not good, assuming all pins are on botL layer.
			{
				...
				if(layer < numLayers)
                	routedGrid.insert(parser::Routed3DPoint(p.x, p.y, layer, is_horizontal[layer]));
		...
		{	
			...
			for (int layer = nodes[i].botL; layer <= nodes[i].topL; layer++) //this is not good, assuming all pins are on botL layer.
			{
				...
				if(layer < numLayers)
					routedGrid.insert(parser::Routed3DPoint(nodes[i].x, nodes[i].y, layer, is_horizontal[layer]));
			...
			if (treeedge->len > 0) {
				...
				for(int output_cnt = 0; output_cnt < 1; output_cnt++)
				{
					for (i = 0; i <= routeLen; i ++) {
						routedGrid.insert(parser::Routed3DPoint(gridsX[i], gridsY[i], gridsL[i], is_horizontal[gridsL[i]]));
						//routedGrid.insert(Point3D(gridsX[i], gridsY[i], gridsL[i]));
						if(i % 5 == 3) {
							if(gridsL[i] <= numLayers - 2) {
								routedGrid.insert(parser::Routed3DPoint(gridsX[i], gridsY[i], gridsL[i] + 1, is_horizontal[gridsL[i] + 1]));
...
void writeRoutedGrid_parallel(FILE* fp)
{
	bool prev_merge = false;
	for(int netID = 0; netID < defDB.nets.size(); netID++) {
		string netName = defDB.nets[netID].name;
		std::set<parser::Routed3DPoint>& routedGrid = defDB.nets[netID].guide_points;
		int start_x = -1, start_y = -1, start_z = -1;
		int end_x = -1, end_y = -1, end_z = -1;
		int llx, lly, urx, ury, layer;
...
void writeRoute3D_parallel(char routingfile3D[], parser::grGenerator grGen, Algo algo)
{
	FILE *fp;
	using Point3D = parser::Point3D<int>;
	...
	galois::do_all(galois::iterate((uint32_t) 0, (uint32_t)numValidNets),[&](uint32_t netID) 
	{
		...
		int grNetID = defDB.netName2netidx.find(netName)->second; 
		auto& grNet = grGen.grnets[grNetID];

		std::set<parser::Routed3DPoint>& routedGrid = defDB.nets[grNetID].guide_points;
		//std::set<Point3D> routedGrid;
		...
		{
			TreeEdge edge = sttrees[netID].edges[edgeID];
	...
	if(algo == WRITE_GUIDE_POINT) {
		write_guide_points();
	}
	else {
		writeRoutedGrid_parallel(fp);
	}
	fclose(fp);
...
/*void writeOverflow(string overflowFile, parser::grGenerator grGen)
{
	short *gridsX, *gridsY, *gridsL;
	int netID, d, i,k, edgeID,nodeID,deg, lastX, lastY,lastL, xreal, yreal,l, routeLen;
	TreeEdge *treeedges, *treeedge;
