...
#include "detpart_astar_region.h"
#include "out_of_guide.h"

void readGR(parser::grGenerator& grGen, Algo algo)
{
	FILE *fp;
	...
	max_rudy = plot_rudy(rudy, algo);
	
	if(algo == DRC_MAP) {
		plot_drc_map();
        exit(0);
    }
	region_partition = new float [xGrid * yGrid];
	if(algo == Region || algo == DetPart_Astar_Region || algo == DetPart_Astar_Regular_Region) {
		rudy_region_partition(rudy, algo, region_partition);
		region_stats(nets, numValidNets, region_partition);
		//exit(0);
	}
	float* pin_density = new float [xGrid * yGrid];

	float avg_pin_den = plot_pin_density(pin_density, algo);
...
void runFastRoute(parser::grGenerator grGen, string benchFile, string OutFileName, parser::CongestionMap& congestionMap, 
	galois::StatTimer& breaktimer, int maxMazeRound = 500, Algo algo = Algo::NonDet)
{
    ...
    if (grGen.grid.x > 0 && grGen.grid.y > 0 && grGen.numNets > 0) {
      lefdef_mode = true;
    }
   ...
   int thread_livelock = 0;
    ...
    if(1)
	{
		...
		if(lefdef_mode)
			...
			readFile(benchFile.c_str());
		
		if(algo == OUT_OF_GUIDE) {
			read_guide_points();
			plot_out_of_guide();
			return ;
		}
		breaktimer.stop();
        cout << "===readgr time: " << breaktimer.get() << endl;
        breaktimer.start();
		...
		breaktimer.stop();
        cout << "===pattern route time: " << breaktimer.get() << endl;
        breaktimer.start();
		...
		while(totalOverflow>0)
		{
			...
			//L = 2; 
			std::vector<std::vector<int>> region_part;
			region_part.resize(num_region_partitions);
			roundtimer.start();

			if(i == 1 && (algo == DetPart_Astar_Region || algo == DetPart_Astar_Regular_Region)) {
				undone_filter(done);
				region_stats_schedule_in_maze(nets, numValidNets, region_partition, done, region_part);
			}
			...
			//galois::runtime::profileVtune( [&] (void) {
				...
				switch(algo) {
					...
					case WRITE_GUIDE_POINT:
					case DetPart_Astar_Small: {
						cout << "num of small undone nets: " << n_small_undone <<  endl;
							//if(max_rudy > 45)
							//	max_nets_per_part = 64;
							undone_filter(done, true);
							int nets_per_part;
					...
					case DetPart_Astar_Local: {
						...
						for (int netOrderID = 0; netOrderID < undone_cnt; netOrderID++) {
							...
							if(undone_cnt != numValidNets && netOrderID < 10) 
					...
					case DetPart_Astar_Regular_Region:
					case DetPart_Astar_Region: {
						if(i == 1) {
							mazeRouteMSMDDetPart_Astar_Region(i, enlarge, costheight, ripup_threshold,
											mazeedge_Threshold, false, cost_type, region_part.size(),
											region_part, done, thread_local_storage, region_partition);
							roundtimer.stop();
							cout << " acc time(ms): " << roundtimer.get() << endl;
							roundtimer.start();
						}
			...
			// Keep via pressure light through most convergence so wirelength can
			// stay primary; only tighten strongly near the last overflow tail.
			if (past_cong < 6000) {
				VIA = max(VIA, 2);
			}
			if (past_cong < 1800) {
		...
		breaktimer.stop();
        cout << "===maze time: " << breaktimer.get() << endl;
        breaktimer.start();
		freeRR();
		...
		// Keep a moderate via pressure during assignment to reduce gratuitous
		// layer bouncing without forcing long detours.
		if (past_cong == 0) {
			VIA = max(VIA, 3);
		}
		...
		breaktimer.stop();
        cout << "===3D time: " << breaktimer.get() << endl;
        breaktimer.start();
		if (write_output) {
			writeRoute3D_parallel(routingFile, grGen, algo);
		}
		breaktimer.stop();
        cout << "===write time: " << breaktimer.get() << endl;
        breaktimer.start();
		/*if (needOUTPUT) {
