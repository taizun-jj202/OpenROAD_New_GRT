#ifndef DETPART_ASTAR_REGION_H
#define DETPART_ASTAR_REGION_H


Bool newRipupCheck_atomic_region(TreeEdge* treeedge, int ripup_threshold, int netID,
  ...
  if (treeedge->route.type == MAZEROUTE) {
    ...
    for (i = 0; i < treeedge->route.routelen; i++) {
        if (gridsX[i] == gridsX[i + 1]) // a vertical edge
        {
            ...
            int old_usage = v_edges[grid].usage + v_local_usage[grid];
      ...
      else if (gridsY[i] == gridsY[i + 1]) // a horizontal edge
      {
            ...
            int old_usage = h_edges[grid].usage + h_local_usage[grid];
...
void mazeRouteMSMDDetPart_Astar_Region(int iter, int expand, float costHeight,
  ...
  std::cout << "inside mazeRouteMSMDDetPart_Astar_Region astar_weight: " << astar_weight << endl;
  ...
  galois::on_each([&] (const unsigned tid, const unsigned numT) {
    ...
    for (int part_idx = tid; part_idx < parts; part_idx += numT) {
      if(region_part[part_idx].size() == 0)
        continue;

      cout << " part: " << part_idx << " " << region_part[part_idx].size() << endl;
      for(int local_id = 0; local_id < region_part[part_idx].size(); local_id++) {
          
          int nidRPC = region_part[part_idx][local_id];
          //cout << "seg fault? " << nidRPC << endl;
		      int grid, netID;
          ...
          std::vector<Usage_Update> usage_updates;
                ...
                mazeedge_Threshold) // only route the non-degraded edges (len>0)
            {
              ...
              enter = newRipupCheck_atomic_region(
              ...
              if (enter) {
                ...
                int curr_color = color_map[ind1];
                //cout << "curr color: " << curr_color << endl;
                pq1.pop();
                curX = ind1 % xGrid;
                curY = ind1 / xGrid;
                              ...
                              // from both heap1 and heap2
                {
                  ...
                  if (curX > regionX1) {
                    ...
                    //cout << "left color: " << color_map[curY * xGrid + tmpX] << endl;
                    if (d1[curY][tmpX] > tmp && // left neighbor been put into heap1 but needs update
                      (OBS_NO_STOP || h_edges[grid].cap > 0) && // hard stop for OBS
                      color_map[curY * xGrid + tmpX] == curr_color) //only route in the same color region
                  ...
                  if (curX < regionX2) {
                    ...
                    //cout << "right color: " << color_map[curY * xGrid + tmpX] << endl;
                  ...
                  if (curY > regionY1) {
                    ...
                    //cout << "bot color: " << color_map[tmpY * xGrid + curX] << endl;
                    if (d1[tmpY][curX] > tmp && // bottom neighbor been put into heap1 but needs update
                      (OBS_NO_STOP || v_edges[grid].cap > 0 ) && //hard stop for OBS
                      color_map[tmpY * xGrid + curX] == curr_color) 
                    {
                  ...
                  if (curY < regionY2) {
                    ...
                    //cout << "top color: " << color_map[tmpY * xGrid + curX] << endl;
                    if(d1[tmpY][curX] > tmp && // top neighbor been put into heap1 but needs update
                      (OBS_NO_STOP || v_edges[grid].cap > 0) && // hard stop for OBS
                      color_map[tmpY * xGrid + curX] == curr_color) //only route in the same color region
                    {
                      float dst_dist = local_astar_weight * (fabs(curX - dst_x) + fabs(tmpY - dst_y));
                      d1[tmpY][curX]       = tmp;
                      parentX1[tmpY][curX] = curX;
        ...
        for(auto usage_update : usage_updates){
