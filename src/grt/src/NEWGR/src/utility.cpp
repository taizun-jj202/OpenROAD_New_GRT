...
void FastRouteCore::fillVIA()
{
  ...
  for (const int& netID : net_ids_) {
    ...
    for (int edgeID = 0; edgeID < sttrees_[netID].num_edges(); edgeID++) {
      ...
      if (treeedge->len > 0) {
        ...
        if (routeLen <= 0) {
          logger_->error(GNR, 254, "Edge has no previous routing.");
        }
  ...
  if (verbose_) {
    logger_->info(GNR, 197, "Via related to pin nodes: {}", numVIAT1);
    logger_->info(GNR, 198, "Via related Steiner nodes: {}", numVIAT2);
    logger_->info(GNR, 199, "Via filling finished.");
  }
}
                               ...
                               const bool is_via_opt_reroute)
{
  ...
  std::vector<int> preferred_layers(routelen, -1);
  std::vector<bool> strong_preference(routelen, false);
  constexpr double via_penalty_scale = 0.5;
  constexpr double via_penalty_slope = 1.5;
  constexpr int preferred_layer_penalty_factor = 4;
  ...
  auto congestionPenalty = [&](int from_layer,
    ...
    const double logistic
        = 1.0 / (1.0 + std::exp(-6.0 * (avg_ratio - 1.0)));
    const double ratio_penalty = std::max(0.0, avg_ratio - 0.9);
    const double penalty_scale = logistic + 0.5 * ratio_penalty;
    if (penalty_scale <= 0.0) {
      return 0;
    }
    return static_cast<int>(std::round(base_cost * penalty_scale));
  };

  auto logisticPenalty = [&](int layer, int grid_idx) -> double {
    if (routelen <= 0 || layer < 0 || layer >= num_layers_) {
      return 0.0;
    }
    grid_idx = clamp_grid_idx(grid_idx);
    const int available = layer_grid[layer][grid_idx];
    if (available <= std::numeric_limits<int>::min() / 4) {
      return 1.0;
    }
    const double demand
        = static_cast<double>(
  ...
  auto congestionCost = [&](int from_layer,
                            int to_layer,
                            int grid_idx,
                            int base_cost) -> int {
    if (routelen <= 0 || base_cost == 0) {
      return 0;
    }
    const double congestion
        = logisticPenalty(from_layer, grid_idx)
          + logisticPenalty(to_layer, grid_idx);
    return static_cast<int>(
        std::round(base_cost * via_penalty_scale * congestion));
  };

  auto preferredViaPenalty = [&](int from_layer,
                                 int to_layer,
                                 int grid_idx) -> int {
    if (grid_idx < 0 || grid_idx >= routelen) {
      return 0;
    }
    const int pref_layer = preferred_layers[grid_idx];
    if (pref_layer < 0 || !strong_preference[grid_idx]) {
      return 0;
    }
    if (from_layer == pref_layer && to_layer == pref_layer) {
      return 0;
    }
    static bool logged_preference_guard = false;
    if (!logged_preference_guard) {
      std::printf("NEW_ALGO_LOGIC_ACTIVE: Preferred-layer via guard engaged\n");
  ...
  const double optimization_trigger
      = 0.7 * static_cast<double>(layer_assign_total_iters_snapshot_);
  ...
  for (k = 0; k < routelen; k++) {
    ...
    int second_best_capacity = std::numeric_limits<int>::min();
    ...
    if (grids[k].x == grids[k + 1].x) {
      ...
      for (int l = net->getMinLayer(); l <= net->getMaxLayer(); l++) {
        ...
        if (is_vertical) {
          ...
          if (layer_grid[l][k] > best_cost) {
            second_best_capacity = best_cost;
            best_cost = layer_grid[l][k];
            best_layer_idx = l;
          } else if (layer_grid[l][k] > second_best_capacity) {
            second_best_capacity = layer_grid[l][k];
          }
      ...
      preferred_layers[k] = best_layer_idx;
      if (best_layer_idx >= 0
          && best_cost > std::numeric_limits<int>::min() / 4
          && (best_cost - second_best_capacity)
                 >= net->getLayerEdgeCost(best_layer_idx)) {
        strong_preference[k] = true;
      }
  ...
  if (processDIR) {
    if (treenodes[n1a].assigned) {
      ...
      if (verbose_) {
        logger_->warn(GNR, 200, "Start point not assigned.");
      }
      fflush(stdout);
    }
    ...
    for (k = 0; k < routelen; k++) {
      for (int l = 0; l < num_layers_; l++) {
        for (int i = 0; i < num_layers_; i++) {
          ...
          int total_via_cost = static_base_cost + base_via_cost
                               + via_resistance_cost
                               + congestionCost(l,
                                                i,
                                                penalty_idx,
                                                base_via_cost)
                               + congestion_penalty
                               + congestionPenalty(l, i, k, base_via_cost)
    ...
    for (int l = 0; l < num_layers_; l++) {
      for (int i = 0; i < num_layers_; i++) {
        ...
        const int congestion_penalty
            = computeCongestionPenalty(
                penalty_idx, base_via_cost, congestion_penalty_weight);
        int total_cost = static_base_cost + base_via_cost + via_resistance_cost
                         + congestionCost(l,
                                          i,
                                          penalty_idx,
                                          base_via_cost)
                         + congestion_penalty
                         + congestionPenalty(l, i, routelen - 1, base_via_cost)
                         + preferredViaPenalty(l, i, penalty_idx);
        ...
        for (int i = 0; i < num_layers_; i++) {
          ...
          int total_via_cost = static_base_cost + base_via_cost
                               ...
                               + congestionCost(l,
                                                ...
                                                base_via_cost)
                               + congestion_penalty
                               + congestionPenalty(l, i, k - 1, base_via_cost)
                               + preferredViaPenalty(l, i, penalty_idx);
      ...
      for (int i = 0; i < num_layers_; i++) {
        ...
        int total_cost = static_base_cost + base_via_cost + via_resistance_cost
                         + congestionCost(l,
                                          ...
                                          base_via_cost)
                         + congestion_penalty
                         + congestionPenalty(l, i, 0, base_via_cost)
                         + preferredViaPenalty(l, i, penalty_idx);
    ...
    for (int nodeID = 0; nodeID < sttrees_[netID].num_nodes(); nodeID++) {
      if (nodeID < num_terminals) {
        ...
        if ((treenodes[nodeID].botL > nets_[netID]->getPinL()[pin_idx])
            || (treenodes[nodeID].topL < nets_[netID]->getPinL()[pin_idx])) {
          logger_->error(GNR, 203, "Caused floating pin node.");
        }
      }
    }
    for (int edgeID = 0; edgeID < sttrees_[netID].num_edges(); edgeID++) {
      ...
      if (grids[0].x != x1 || grids[0].y != y1) {
        debugPrint(
            ...
            num_terminals,
            n1);
        if (logger_->debugCheck(GNR, "checkRoute3D", 1)) {
          printEdge3D(netID, edgeID);
        }
      }
      ...
      for (int i = 0; i <= treeedge->route.routelen; i++) {
        if (grids[i].layer < 0) {
          logger_->error(
              GNR, 204, "Invalid layer value in gridsL, {}.", grids[i].layer);
        }
      }
      if (gridFlag && logger_->debugCheck(GNR, "checkRoute3D", 1)) {
        printEdge3D(netID, edgeID);
      }
    }
  }
...
void FastRouteCore::recoverEdge(const int netID, const int edgeID)
{
  ...
  if (treeedge->len == 0) {
    logger_->error(GNR, 206, "Trying to recover a 0-length edge.");
  }
...
bool FastRouteCore::checkRoute2DTree(const int netID)
{
  ...
  for (int edgeID = 0; edgeID < sttrees_[netID].num_edges(); edgeID++) {
    ...
    if (treeedge->len < 0) {
      if (verbose_) {
        logger_->warn(
            GNR, 207, "Ripped up edge without edge length reassignment.");
      }
      STHwrong = true;
    }
    ...
    if (treeedge->len > 0) {
      ...
      if (STHwrong) {
        if (verbose_) {
          logger_->warn(
              GNR, 167, "Invalid 2D tree for net {}.", nets_[netID]->getName());
        }
        return true;
      }
    }
...
int FastRouteCore::edgeShift(Tree& t, const int net)
{
  ...
  for (int i = 0; i < deg; i++) {
    ...
    if (n >= deg && n < t.branchCount()) {  // ensure n is inside nbrCnt range
      ...
      logger_->error(GNR, 149, "Invalid access to nbrCnt vector");
    }
  }
