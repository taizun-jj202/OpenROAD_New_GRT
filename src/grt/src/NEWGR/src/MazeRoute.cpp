...
void MazeRoute::run()
{
  ...
  auto edgeCostBetweenVertices = [&](int u, int v) {
    const CostT invalidCost = std::numeric_limits<CostT>::max() / 8;
    for (int edgeIndex = 0; edgeIndex < 3; edgeIndex++) {
  ...
  auto runMetricClosureMstCandidate =
      [&](std::vector<std::shared_ptr<Solution>>& output) {
        output.clear();
        const int numPins = graph_.getNumPseudoPins();
        const int numVertices = graph_.getNumVertices();
        if (numPins < 2 || numVertices <= 0) {
          return false;
        }

        const CostT invalidCost = std::numeric_limits<CostT>::max() / 8;
        const CostT eps = 1e-9;

        std::vector<int> pinVertices(numPins, -1);
        ...
        std::vector<std::vector<CostT>> pairCost(
            numPins, std::vector<CostT>(numPins, invalidCost));
        std::vector<std::vector<int>> prevBySource(
            numPins, std::vector<int>(numVertices, -1));
        std::vector<CostT> dist(numVertices, invalidCost);

        auto runDijkstra = [&](int sourceVertex,
                               std::vector<CostT>& distances,
                               std::vector<int>& prev) {
          distances.assign(numVertices, invalidCost);
          prev.assign(numVertices, -1);

          using QueueNode = std::pair<CostT, int>;
          auto compareNode = [](const QueueNode& lhs, const QueueNode& rhs) {
            return lhs.first > rhs.first;
          };
          std::priority_queue<QueueNode,
                              std::vector<QueueNode>,
                              decltype(compareNode)>
              queue(compareNode);
          ...
          while (!queue.empty()) {
            ...
            for (int edgeIndex = 0; edgeIndex < 3; edgeIndex++) {
              ...
              if (edgeCost >= invalidCost) {
                continue;
              }
              const CostT nextCost = cost + edgeCost;
              if (nextCost + eps < distances[nextVertex]) {
                distances[nextVertex] = nextCost;
        ...
        int rootPin = -1;
        CostT bestRootCost = invalidCost;
        for (int sourcePin = 0; sourcePin < numPins; sourcePin++) {
          runDijkstra(pinVertices[sourcePin], dist, prevBySource[sourcePin]);
          CostT totalDistance = 0;
          bool reachable = true;
          for (int targetPin = 0; targetPin < numPins; targetPin++) {
            const CostT d = dist[pinVertices[targetPin]];
            pairCost[sourcePin][targetPin] = d;
            if (d >= invalidCost) {
              reachable = false;
              break;
            }
          ...
          if (reachable && totalDistance < bestRootCost) {
            bestRootCost = totalDistance;
            rootPin = sourcePin;
          }
        }
        if (rootPin < 0) {
        ...
        std::vector<bool> inTree(numPins, false);
        std::vector<CostT> bestAttachCost(numPins, invalidCost);
        std::vector<int> parent(numPins, -1);
        bestAttachCost[rootPin] = 0;
        for (int iter = 0; iter < numPins; iter++) {
          int pick = -1;
          CostT pickCost = invalidCost;
          for (int pin = 0; pin < numPins; pin++) {
            if (!inTree[pin] && bestAttachCost[pin] < pickCost) {
              pick = pin;
              pickCost = bestAttachCost[pin];
            }
          }
          if (pick < 0 || pickCost >= invalidCost) {
            return false;
          }
          inTree[pick] = true;
          for (int pin = 0; pin < numPins; pin++) {
            ...
            const CostT edgeCost = pairCost[pick][pin];
            if (edgeCost < bestAttachCost[pin]) {
              bestAttachCost[pin] = edgeCost;
              parent[pin] = pick;
            }
          }
        }

        std::vector<std::vector<int>> mstChildren(numPins);
        for (int pin = 0; pin < numPins; pin++) {
          if (pin == rootPin) {
            continue;
          }
          ...
          mstChildren[parent[pin]].push_back(pin);
        }

        std::vector<int> visitOrder;
        visitOrder.reserve(numPins);
        visitOrder.push_back(rootPin);
        for (size_t idx = 0; idx < visitOrder.size(); idx++) {
          const int pin = visitOrder[idx];
          for (const int child : mstChildren[pin]) {
            visitOrder.push_back(child);
          }
        }

        output.reserve(std::max(0, numPins - 1));
        for (int orderIdx = 1; orderIdx < static_cast<int>(visitOrder.size());
             orderIdx++) {
          const int childPin = visitOrder[orderIdx];
          const int parentPin = parent[childPin];
          const int childVertex = pinVertices[childPin];
          const int parentVertex = pinVertices[parentPin];
          if (childVertex == parentVertex) {
            continue;
          }

          const auto& prev = prevBySource[childPin];
          std::vector<int> pathVertices;
          int cursor = parentVertex;
          while (cursor != -1) {
            pathVertices.push_back(cursor);
            if (cursor == childVertex) {
              break;
            }
            cursor = prev[cursor];
          }
          if (pathVertices.empty() || pathVertices.back() != childVertex) {
            return false;
          }
          ...
          CostT pathCost = 0;
          for (size_t idx = 1; idx < pathVertices.size(); idx++) {
            const CostT edgeCost
                = edgeCostBetweenVertices(pathVertices[idx - 1], pathVertices[idx]);
            if (edgeCost >= invalidCost) {
              return false;
            }
            pathCost += edgeCost;
            pathSolution = std::make_shared<Solution>(
                pathCost, pathVertices[idx], pathSolution);
          }
          output.emplace_back(pathSolution);
        }
  ...
  const bool runMstCandidate
      = numPseudoPins >= 4 && numPseudoPins <= 28
        && (net_->getNumPins() >= 6 || box.hp() >= 80);
  if (runMstCandidate) {
    std::vector<std::shared_ptr<Solution>> mstSolutions;
    if (runMetricClosureMstCandidate(mstSolutions)) {
